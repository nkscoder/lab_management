(function( $ ){
    $.fn.overlay = function() {
        ...
        if( !$("#jqueryEasyOverlayDiv").length ) {
            ...
            $(innerDiv).css({ position: "absolute",left: "47.5%",top:"47.5%"}).attr("id", "jqueryOverlayLoad")
            .html("<image class='loading' style='height:100px;' src='"+ $("base").attr('href') +"assets/images/loading.gif'>");
        ...
        $("#jqueryEasyOverlayDiv").css({
            top     : $t.offset().top,
            left    : $t.offset().left,
            width   : jQuery(window).width(),//$t.outerWidth(),
            height  : jQuery(window).height(),//$t.outerHeight()
            'z-index' : 9999
        });
...
//extending the global jquery function
//this can called with any element $('#elementid').overlayout();
//which can used globally to any element 
(function($){
    $.fn.overlayout = function() {
        if( $("#jqueryEasyOverlayDiv").length ) $("#jqueryEasyOverlayDiv").fadeOut();
   }; 
})(jQuery);
//it is a normal function which can be called as 
//$.overlayout();

(function($){
    $.overlayout = function() {
        if($("#jqueryEasyOverlayDiv").length) $("#jqueryEasyOverlayDiv").fadeOut('slow');
   }; 
...
(function(window, undefined) {
    ...
    $(function() {
        // Ajaxify links
        $(document).on('click', 'a[rel],button[rel]', function(e) {
            var $a = $(this),
                rel = $a.attr('rel'),
                url = $a.attr('ajaxify');
