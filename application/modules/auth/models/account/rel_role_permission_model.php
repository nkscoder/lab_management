...
class Rel_role_permission_model extends CI_Model {
  ...
  function get()
  {
    return $this->db->get('rel_role_permission')->result();
  }
  ...
  function get_by_role_id($role_id)
  {
    ...
    $this->db->from('rel_role_permission');
    $this->db->join('acl_permission', 'rel_role_permission.permission_id = acl_permission.id');
    $this->db->where("rel_role_permission.role_id = $role_id AND acl_permission.suspendedon IS NULL");
  ...
  function get_by_permission_id($permission_id)
  {
    ...
    $this->db->from('rel_role_permission');
    $this->db->join('acl_role', 'rel_role_permission.role_id = acl_role.id');
    $this->db->where("rel_role_permission.permission_id = $permission_id AND acl_role.suspendedon IS NULL");
  ...
  function update($role_id, $permission_id)
  {
    ...
    {
      $this->db->insert('rel_role_permission', array('role_id' => $role_id, 'permission_id' => $permission_id));
    }
  }
  ...
  function update_batch($role_id, $permission_ids)
  {
    ...
    if( count($permission_ids) > 0)
    {
      ...
      $this->db->insert_batch('rel_role_permission', $batch);
    }
  }
  ...
  {
    $this->db->delete('rel_role_permission', array('role_id' => $role_id, 'permission_id' => $permission_id));
  }
  ...
  {
    $this->db->delete('rel_role_permission', array('role_id' => $role_id));
  }
  ...
  {
    $this->db->delete('rel_role_permission', array('permission_id' => $permission_id));
  }
}