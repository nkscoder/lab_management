...
class Manage_permissions extends MY_Controller {
  ...
  function __construct()
  {
    ...
    if(!$this->authorization->is_permitted('manage_permissions') && !$this->authorization->is_admin()){
        redirect('');
    }
  ...
  function index()
  {
    ...
    $this->data['appScript'] = "$('#permissions_list').DataTable();";
    ...
    $this->template->load_view('account/manage_permissions', $this->data);
  }
  ...
  function save($id=null)
  {
    ...
    if ( ! $this->authentication->is_signed_in())
    {
      redirect('auth/account/sign_in/?continue='.urlencode(base_url().'account/manage_permissions'));
    }
    ...
    if( ! $is_new )
    {
      $this->data['permission'] = $this->acl_permission_model->get_by_id($id);
      // dump_exit($this->data['permission']);
      if(empty($this->data['permission'])){
        redirect('auth');
      }
      
      $this->data['role_permissions'] = $this->rel_role_permission_model->get_by_permission_id($id);
      $this->data['action'] = 'update';
      $this->data['is_system'] = ($this->data['permission']->is_system == 1);
    }
    ...
    if ($this->form_validation->run())
    {
      ...
      if ( (! empty($id) && strtolower($this->input->post('permission_key', TRUE)) != strtolower($this->data['permission']->key) && $name_taken) || (empty($id) && $name_taken) )
      {
        $this->data['permission_key_error'] = lang('permissions_name_taken');
      }
      else
      {
        // Create/Update role
        ...
        redirect('auth/account/manage_permissions');
      }
    }
    // Load manage permissions view
    $this->template->load_view('account/manage_permissions_save', $this->data);
  }
