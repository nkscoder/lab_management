...
class Manage_users extends MY_Controller {
  ...
  function __construct()
  {
    ...
    if(!$this->authorization->is_permitted('manage_users') && !$this->authorization->is_admin()){
        redirect('');
    }
  ...
  { 
    ...
    if($roleid)
      $all_accounts = $this->account_model->get_by_role($roleid);
    else
    ...
    foreach ( $all_accounts as $acc )
    {
      ...
      // Append to the array
      $this->data['all_accounts'][] = $current_user;
    }

    

    $this->data['appScript'] = "$('#users_list').DataTable();";
    ...
    $this->template->load_view('account/manage_users', $this->data);
  }
  ...
  function save($id=null)
  {
    ...
    if ( ! $this->authentication->is_signed_in())
    {
      redirect('auth/account/sign_in/?continue='.urlencode(base_url().'account/manage_users'));
    }
    ...
    // Get all the roles
    $this->data['roles'] = $this->acl_role_model->get();
    ...
    if( ! $is_new )
    {
      $this->data['update_account'] = $this->account_model->get_by_id($id);

      if(empty($this->data['update_account'])){
        redirect('auth');
      }
      
      $this->data['update_account_details'] = $this->account_details_model->get_by_account_id($id);
      $this->data['update_account_roles'] = $this->acl_role_model->get_by_account_id($id);
    ...
    if ($this->form_validation->run())
    {
      ...
      if ( (! empty($id) && strtolower($this->input->post('users_email', TRUE)) != strtolower($this->data['update_account']->email) && $email_taken) || (empty($id) && $email_taken) )
      {
        $this->data['users_email_error'] = lang('settings_email_exist');
      }
      // Check if user name is taken
      elseif ( (! empty($id) && strtolower($this->input->post('users_username', TRUE)) != strtolower($this->data['update_account']->username) && $username_taken) || (empty($id) && $username_taken) )
      {
        $this->data['users_username_error'] = lang('sign_up_username_taken');
      }
      else
      {
        ...
        redirect("auth/account/manage_users");
      }
    }

    // Load manage users view
    $this->template->load_view('account/manage_users_save', $this->data);
  }
