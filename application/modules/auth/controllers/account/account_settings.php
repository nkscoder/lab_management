...
class Account_settings extends MY_Controller {
	...
	function index()
	{
		...
		if ($this->form_validation->run())
		{
			...
			if (strtolower($this->input->post('settings_email', TRUE)) != strtolower($this->data['account']->email) && $this->email_check($this->input->post('settings_email', TRUE)) === TRUE)
			{
			...
			else
			{
				...
				$this->account_model->update_email($this->data['account']->id, $this->input->post('settings_email', TRUE) ? $this->input->post('settings_email', TRUE) : NULL);
				...
				$this->data['settings_info'] = lang('settings_details_updated');
			}
		}

		$this->template->load_view('account/account_settings', $this->data);
	}
