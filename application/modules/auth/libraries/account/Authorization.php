...
class Authorization {
  ...
  function is_permitted($permission_keys, $require_all = FALSE)
  {
    ...
    $this->CI->load->model('auth/account/acl_permission_model');
    ...
    if (isset($permission_keys))
    {
      foreach ($account_permissions as $perm) 
      {
        ...
        {
          // Return if only a single one is required.
          if ($perm->key == $permission_keys && ! $require_all && !$perm->system_access_only ) 
          {
            return TRUE;
          }
          // Only takes one bad apple
  ...
  function is_admin()
  {
    ...
    $this->CI->load->model('auth/account/acl_role_model');
  ...
  function is_staff()
  {
    ...
    return $this->CI->acl_role_model->has_role('staff', $account_id);
  }
  ...
  function is_user()
  {
    ...
    return $this->CI->acl_role_model->has_role('User', $account_id);
  }
