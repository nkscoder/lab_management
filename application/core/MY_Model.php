...
class MY_Model extends CI_Model
{
    ...
    /**
     *  Delete many rows from the database table an arbitrary WHERE clause
     */
    ...
    public function validateOverRide($nrules = array(),$key = '')
    {   
        foreach($nrules as $k => $v)
        {   

            foreach($this->validate as $ok => $or)
            {
                if($or['field'] == $k)  //found rules
                {
                    $this->validate[$ok] = array_merge($this->validate[$ok],$v);
                    break;
                }
                else
                {
